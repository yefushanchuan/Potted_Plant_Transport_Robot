...
> **提示**：`points` 数组采用世界坐标（map frame，米），矩形最少两个点， 多边形至少三个点。可与 App 侧协议保持一致，后续也能通过 MQTT 增量同步。

### 3. 启动导航（已包含 keepout 支持）

```bash
ros2 launch robot_nav nav03_navigation.launch.py
ros2 launch robot_nav nav07_keepout_filter.launch.py
```
...
当前实现依赖 `nlohmann_json` 头文件库（ROS 发行版通常已包含）。若出现头文件缺失，可通过以下命令安装：
```bash
sudo apt-get update
