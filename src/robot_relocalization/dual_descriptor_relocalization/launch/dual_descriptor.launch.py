...
def _launch_setup(context):
    ...
    dual_desc_params_file = os.path.join(
        get_package_share_directory('dual_descriptor_relocalization'),
        'config',
        'dual_descriptor.yaml'
    )
    ...
    return [
        ...
        # ── Dual Descriptor 全局重定位节点（开机搜索, 之后静默） ──
        Node(
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('rviz_config_path',
                              default_value=PathJoinSubstitution([
                                  FindPackageShare('indoor_location'),
                                  'config/rviz/livox_rviz_config.rviz'
                              ])),
        DeclareLaunchArgument('enable_icp_registration', default_value='true',
                              description='Whether to launch icp_registration node'),
