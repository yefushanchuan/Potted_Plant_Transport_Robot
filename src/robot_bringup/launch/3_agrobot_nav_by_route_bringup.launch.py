...
def generate_launch_description():
    ...
    nav2_rewritten_yaml = RewrittenYaml(
        ...
        param_rewrites={
            ...
            "bt_navigator.ros__parameters.default_nav_to_pose_bt_xml": PathJoinSubstitution(
                [pkg_robot_bringup, "config", "navigation", "nav_to_pose_w_route_tree.xml"]
            ),
        },
        convert_types=True,
    )
    ...
    default_map_file = PathJoinSubstitution([pkg_robot_bringup, "map", "map.yaml"])
    default_graph_file = os.path.join(pkg_robot_bringup, "route", "test_route.yaml")
    default_rviz_config = PathJoinSubstitution([pkg_robot_bringup, "rviz", "nav.rviz"])
    default_nav2_params = PathJoinSubstitution([pkg_robot_bringup, "config", "navigation", "Nav_route.yaml"])
