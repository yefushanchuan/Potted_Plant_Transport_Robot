...
def generate_launch_description():
    ...
    default_bt_xml = PathJoinSubstitution([
        pkg_robot_bringup, 'config', 'behavior_trees', 'main_tree', 'bdh_tree.xml'
    ])
    ...
    param_specs = [
        ...
        ('error_log_file', 'src/robot_bringup/log/error_log.txt', 'LogErrorCondition 错误日志输出路径'),
        # 运行状态参数
        ('task_locked', 'false', '任务锁状态(true 表示当前任务执行中）'),
        ('target_pot_num', '0', '目标盆栽编号'),
        ('error_message', 'IDLE', '初始错误信息'),
        ('robot_state', 'IDLE', '机器人初始状态'),
        ('bt_loop_duration', '20', 'BT Tick 周期（毫秒）'),
        ...
        ('taskfile_retry_queue_file', 'src/robot_bringup/log/taskfile_retry_queue.txt', '任务文件上传失败重试队列持久化文件'),
    ]
